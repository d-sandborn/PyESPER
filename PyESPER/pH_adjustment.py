def pH_adjustment(
    ...
    **kwargs
    ...
    if "EstDates" in kwargs and (
            ...
            for combo, values in zip(combos2, values2):
                if combo.startswith("pH"):
                    ...
                    InputAll2 = inputdata_organize(
                        Dates, C, PM_pH, Uncertainties_pre
                    )
                    PredictorMeasurements2, InputAll2 = temperature_define(
                        ["TA"], PM_pH, InputAll2, **kwargs
                    )
                    ...
                    aaLCs, aaInterpolants_pre, elLCs, elInterpolants_pre = (
                        interpolate(Path, Gdf, AAdata, Elsedata, verbose=False)
                    )
        ...
        and not VerboseTF
        and YouHaveBeenWarnedCanth
    ):
        print(
            "Warning: DIC or pH is a requested output but the user did not provide dates for the desired esimtates. The estimates "
